      ...
      <main className="max-w-3xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex-1">
        <div className="mb-8">
          ...
          <p className="text-zinc-400 text-xs sm:text-sm mt-1">
            Define la identidad, el nivel pedagógico y las políticas de respuesta de tu asistente educativo.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card-clean rounded-xl p-6 sm:p-8 space-y-6 border border-zinc-800/80 bg-zinc-900/30">
          <div className="space-y-4">
            <div>
              <label htmlFor="name" className="block text-xs font-medium text-zinc-300 mb-1.5 flex items-center gap-1.5">
                Nombre del tutor *
                <HelpTooltip text="Nombre con el que tus estudiantes identificarán a este tutor. Ej: Tutor de Matemáticas 101." />
              </label>
              <input
                id="name"
                name="name"
                ...
                className="w-full px-3.5 py-2.5 bg-zinc-900 border border-zinc-800 rounded-lg focus:ring-1 focus:ring-zinc-600 focus:border-zinc-600 outline-none text-xs sm:text-sm text-zinc-100 placeholder-zinc-500 transition-colors font-sans"
            ...
            <div>
              <label htmlFor="subject_area" className="block text-xs font-medium text-zinc-300 mb-1.5 flex items-center gap-1.5">
                ...
                {([
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-lg border text-left transition-colors flex flex-col gap-1 cursor-pointer focus:outline-none focus:ring-1 focus:ring-zinc-600 btn-press ${
                        selected
                          ? "border-zinc-600 bg-zinc-800 text-zinc-100"
                          : "border-zinc-800 bg-zinc-900/40 text-zinc-400 hover:border-zinc-700 hover:text-zinc-200"
                      }`}
                    >
                      <h4 className="font-medium text-xs text-zinc-100 flex items-center justify-between">
                        {opt.label}
                        {selected && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />}
                      </h4>
                      <p className={`text-[11px] leading-tight ${selected ? "text-zinc-300" : "text-zinc-500"}`}>
                        {opt.desc}
                      </p>
                    </button>
                  );
                ...
                {([
                  ...
                  return (
                    <button
                      ...
                      className={`p-2.5 rounded-lg border text-left transition-colors flex flex-col cursor-pointer focus:outline-none focus:ring-1 focus:ring-zinc-600 btn-press ${
                        selected
                          ? "border-zinc-600 bg-zinc-800 text-zinc-100"
                          : "border-zinc-800 bg-zinc-900/40 text-zinc-400 hover:border-zinc-700 hover:text-zinc-200"
                      }`}
                    >
                      <h4 className="font-medium text-xs text-zinc-100">{t.label}</h4>
                      <p className={`text-[11px] mt-1 ${selected ? "text-zinc-300" : "text-zinc-500"}`}>{t.desc}</p>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 pt-2">
              ...
              <div className="grid grid-cols-3 gap-2" role="radiogroup" aria-label="Nivel de restricción">
                {([
                  ...
                  return (
                    ...
                    >
                      <h4 className="font-medium text-xs text-zinc-100">{r.label}</h4>
                      <p className={`text-[11px] mt-1 ${selected ? "text-zinc-300" : "text-zinc-500"}`}>{r.desc}</p>
                    </button>
                  );
                })}
              </div>
            ...
            <div>
              <label className="block text-xs font-medium text-zinc-300 mb-2 flex items-center gap-1.5">
                Motor LLM
                <HelpTooltip text="Inferencia unificada vía OpenRouter. Modelos abiertos de alta calidad." />
              </label>
              <div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label="Motor LLM">
                ...
                >
                  ...
                  <p className={`text-[11px] mt-1 ${formData.llm_provider === "openrouter" ? "text-zinc-300" : "text-zinc-500"}`}>BYOK · Modelos libres</p>
                </button>
                <button
                  type="button"
                  disabled
                  ...
                  className="p-2.5 rounded-lg border text-left flex flex-col opacity-40 cursor-not-allowed border-zinc-800 bg-zinc-900/20"
                >
                  <h4 className="font-medium text-xs text-zinc-500">Claude Native</h4>
          ...
          <div className="space-y-4 pt-2">
            <div>
              <label htmlFor="welcome_message" className="block text-xs font-medium text-zinc-300 mb-1.5 flex items-center gap-1.5">
              ...
              <textarea
                ...
                onChange={handleChange}
                rows={2}
                className="w-full px-3.5 py-2.5 bg-zinc-900 border border-zinc-800 rounded-lg focus:ring-1 focus:ring-zinc-600 focus:border-zinc-600 outline-none text-xs sm:text-sm text-zinc-100 placeholder-zinc-500 transition-colors font-sans resize-none"
                placeholder="Ej: ¡Hola! Soy el asistente de Cálculo Diferencial. ¿Qué teorema o problema te gustaría resolver hoy?"
              />
            </div>

            <div>
              <label htmlFor="system_prompt_override" className="block text-xs font-medium text-zinc-300 mb-1.5 flex items-center gap-1.5">
                Instrucciones pedagógicas personalizadas (opcional)
                <HelpTooltip text="Directivas adicionales que el modelo seguirá en cada respuesta." />
              </label>
              <textarea
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-zinc-800">
            <button
              type="button"
              onClick={() => router.back()}
              className="btn-press px-4 py-2.5 border border-zinc-800 hover:bg-zinc-900 text-zinc-300 hover:text-zinc-100 rounded-lg font-medium text-xs transition-colors"
            >
              Cancelar
            </button>
            <button
              ...
              className="btn-press flex-1 px-5 py-2.5 bg-zinc-100 hover:bg-white text-zinc-950 rounded-lg font-medium text-xs transition-colors disabled:opacity-50 inline-flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
                <>
